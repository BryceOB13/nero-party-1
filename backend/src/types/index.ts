...
export interface FinalStanding {
  ...
  bonusPoints: number;
  achievementBonus: number;    // Bonus points from achievements
  finalScore: number;
  scoreBreakdown: ScoreBreakdown;  // Detailed score breakdown
...
export interface ScoreBreakdown {
  baseScore: number;           // Sum of weighted song scores
  roundMultiplier: number;     // Average round multiplier applied
  themeBonus: number;          // Bonus from theme adherence
  confidenceModifier: number;  // Sum of confidence modifiers
  powerUpModifier: number;     // Bonus/penalty from power-ups
  eventModifier: number;       // Bonus/penalty from mini-events
  achievementBonus: number;    // Bonus points from achievements
  predictionBonus: number;     // Bonus points from correct predictions
  bonusPoints: number;         // Bonus points from bonus categories
...
export interface BonusCategory {
  ...
  description: string;
  points: number;
  calculate: (songs: Song[]) => Song | null;
}
...
/**
 * Represents a track in the demo playlist.
 * Used for auto-submitting songs in demo mode.
 */
export interface DemoTrack {
  ...
  duration: number;
  permalinkUrl: string;
  expectedScore: number;  // For realistic simulation
}

/**
 * Represents a voter personality for simulating votes in demo mode.
 * Each personality has different voting patterns.
 * 
 * **Validates: Requirements 9.3, 9.6**
 */
export interface VoterPersonality {
  name: string;
  meanRating: number;  // Average rating they give
  variance: number;    // How much they vary
  bias: 'generous' | 'harsh' | 'balanced';
}

/**
 * Demo timing configuration for overriding default settings.
 * 
 * **Validates: Requirements 9.4, 9.5**
 */
export interface DemoTimingConfig {
  playDuration: number;  // Override play duration (15 seconds in demo)
  finaleAnimationSpeed: number;  // Animation speed multiplier (2x in demo)
}

/**
 * Default demo timing configuration.
 * - play_duration: 15 seconds (Requirement 9.4)
 * - finale animation speed: 2x (Requirement 9.5)
 */
export const DEMO_TIMING_CONFIG: DemoTimingConfig = {
  playDuration: 15,
  finaleAnimationSpeed: 2.0,
};

/**
 * Predefined voter personalities for demo mode.
 * 
 * **Validates: Requirements 9.3, 9.6**
 * - Harsh Critic: Lower mean rating, moderate variance
 * - Generous Fan: Higher mean rating, lower variance
 * - Balanced Voter: Middle mean rating, moderate variance
 * - Wildcard: Middle mean rating, high variance
 */
export const DEMO_PERSONALITIES: VoterPersonality[] = [
  { name: 'Harsh Critic', meanRating: 5.5, variance: 2.0, bias: 'harsh' },
  { name: 'Generous Fan', meanRating: 8.0, variance: 1.5, bias: 'generous' },
  { name: 'Balanced Voter', meanRating: 6.5, variance: 2.5, bias: 'balanced' },
  { name: 'Wildcard', meanRating: 6.0, variance: 4.0, bias: 'balanced' }
];
...
/**
 * Result of applying a power-up effect.
...
export interface PowerUpEffectResult {
  ...
  message: string;
  data?: {
    vote?: { voterId: string; rating: number };
    identity?: { playerId: string; realName: string };
    multiplier?: number;
    protected?: boolean;
    eligible?: boolean;
  };
